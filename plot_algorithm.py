...
def plot_re_pre_day(data):
    ...
    # 画图
    # 召回率的点线图
    plt.plot(week_days, recall_list[1:], color='orange', marker='o', label="Recall")
    # 精确率的点线图
    plt.plot(week_days, precision_list[1:], color='dodgerblue', marker='^', label="Precision")
    # 召回率的均值
    ...
    plt.legend()
    # 标题
    plt.title('本周平均Recall: ' + str('%.2f%%' % recall_list[0]) + ' 本周平均Precision: ' + str('%.2f%%' % precision_list[0]))
    ...
    plt.savefig('algorithm_re_pre.png', bbox_inches='tight')
    plt.show()
    return
